...
class TwoHeadGapFinder:
    """Shared transformer with gap-regression and tail-detection outputs."""

    METADATA_FILE_NAME = "two_head_gap_finder_metadata.json"

    def __init__(
        self,
        model_name: str,
        theta: float,
        gap_finder_id: str | None = None,
        ...
        self.theta = theta
        self.id = gap_finder_id or str(uuid4())
    ...
    def predict(
        ...
        *,
        ...
        """Return predicted gaps and probabilities that the gap exceeds theta."""
        ...
        for start in range(0, len(prompts), batch_size):
            ...
            if logits.ndim != 2 or logits.shape[1] != 2:
                raise ValueError(
                    f"TwoHeadGapFinder expected [batch, 2] logits, got {tuple(logits.shape)}"
                )
            batch_gaps = logits[:, 0].cpu().tolist()
            batch_probabilities = torch.sigmoid(logits[:, 1]).cpu().tolist()
            if not all(
                math.isfinite(value)
                for value in batch_gaps + batch_probabilities
            ):
                raise ValueError("TwoHeadGapFinder returned a non-finite prediction")
            gaps.extend(batch_gaps)
            probabilities.extend(batch_probabilities)
        return gaps, probabilities
        ...
        *,
        ...
        if not checkpoint.is_dir():
            raise FileNotFoundError(
                f"TwoHeadGapFinder checkpoint not found: {checkpoint}"
            )
        metadata_path = checkpoint / cls.METADATA_FILE_NAME
        metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
        if metadata.get("format_version") != 1:
            raise ValueError(
                f"Unsupported TwoHeadGapFinder metadata: {metadata_path}"
            )
        ...
        loaded.model.eval()
        loaded.theta = float(metadata["theta"])
